...
const Register: NextPage = () => {
  ...
  const additionalLinks = (
    <>
      ...
      <Text mt={2} textAlign={['center']}>
        Forgot password?{' '}
        <CLink color="brand.600" as="strong" role="reset-password">
  ...
  const handleOnSubmit = async (values: RegisterInputs, setFieldError: Function) => {
    console.log(values);
  ...
  return (
    <AuthLayout additionalLinks={additionalLinks} tabTitle="Register">
      <Formik initialValues={{ username: '', password: '', confirmPassword: '', email: '' }} onSubmit={(values: RegisterInputs, { setFieldError }) => handleOnSubmit(values, setFieldError)}>
        {({ isSubmitting }) => (
          <Form>
